...
def get_gene_uniprot_file(strain, result_file, tag, mean_tag, new_proteome_file):
    if tag == "new":
        proteome_file = new_proteome_file
    else:
        ...
        if tag !="0":
            proteome_file = transcriptome_set_data(proteome_file, tag, mean_tag, strain)
    
    gene_uniprot_outfile='./download_data/Gene_name2ACC_MW_%s.txt'%strain
    if not os.path.exists(gene_uniprot_outfile):
    ...
    if not os.path.exists(gene_uniprot_select_outfile):
...
def transcriptome_set_data(proteome_file, tag, mean_tag, strain):
    if mean_tag =="F":
        proteome_file_new = proteome_file.split('.csv')[0] + "_"+tag + '.csv'
        print("proteome_file_new:",proteome_file_new)
    elif mean_tag =="T":
        # '../basic_data/omics_data/random_mean/MG1655_trans_transcriptome_mean.csv'
        new_folder = proteome_file.split(strain)[0] +"random_mean/"
        if not os.path.exists(new_folder):    # 创建新文件夹
            os.makedirs(new_folder)

        proteome_file_new = new_folder + strain + proteome_file.split(strain)[1]
        print("proteome_file_new:",proteome_file_new)
    ...
    if tag == "1" and mean_tag=="F":
        ...
        transcriptome_sum_df = pd.DataFrame(arr_1, index=index_list, columns=columns_list)
        if mean_tag =="F":
            transcriptome_sum_df.to_csv(proteome_file_new, header=True, index=True)

        elif mean_tag =="T":
            for n in range(1, 1000):
                arr_1 = np.random.uniform(min_value, max_value, (len_index, len_col))  
                transcriptome_df = pd.DataFrame(arr_1, index=index_list, columns=columns_list)
                transcriptome_sum_df = transcriptome_sum_df + transcriptome_df

            transcriptome_mean_df = transcriptome_sum_df/1000
            transcriptome_mean_df.to_csv(proteome_file_new, header=True, index=True)
...
def analysis_main():
    ...
    parser.add_argument('-mean_tag', '--mean', type=str)
    ...
    if tag !="new":
        ...
        new_proteome_file = "./basic_data/omics_data/Scerevisiae_new/scerevisiae_translation.csv"
    ...
    for strain in strain_list:
        ...
        (proteome_file,gene_uniprot_select_outfile) = get_gene_uniprot_file(strain, result_file, tag, mean_tag, new_proteome_file)
